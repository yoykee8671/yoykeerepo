  ...
  for (const order of orders) {
    ...
    for (const item of order.items || []) {
      ...
      rows.push({
        ...
        // 배송완료일과 배송시작일(shipped_date)은 다르다. "배송완료일 기준" 정산의
        // 정산월을 가르는 값이므로 반드시 delivered_date 를 쓴다. "주문일 기준"
        // 정산은 출고완료(배송완료 또는 송장 등록) 여부만 보고, 그 판정에는 아래
        // 송장번호도 같이 쓴다 — 배송완료 마킹이 늦어도 송장이 찍혔으면 이미 출고된 것.
        "배송완료일": dateOnly(item.delivered_date),
        "송장번호": String(item.tracking_no || ""),
