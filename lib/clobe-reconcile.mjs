// Matches unpaid WooofPay deposit requests against real bank deposits pulled
// from clobe.
//
...
const DEFAULT_EXCLUDED_CATEGORIES = ["계좌간 입금", "계좌간 이체"];
...
export function reconcile({ requests = [], transactions = [], options = {} }) {
  ...
  for (const deposit of deposits) {
    ...
    const named = deposit.orderNos.map((no) => byOrderNo.get(no)).filter(Boolean).filter((r) => !usedRequests.has(r.id));
  ...
  for (const deposit of deposits) {
    ...
    for (const [, group] of groups) {
      const available = group.filter((request) => !usedRequests.has(request.id));
...
function buildMatch(deposit, requests, kind) {
  ...
  if (kind === "memo") {
    ...
    if (total === deposit.amount) {
      reasons.push(`금액도 일치 (${formatMoney(deposit.amount)})`);
    } else {
      reasons.push(`⚠ 금액 불일치 — 입금 ${formatMoney(deposit.amount)} vs 요청 합계 ${formatMoney(total)} (차액 ${formatMoney(deposit.amount - total)})`);
    }
    return {
      kind,
  ...
  if (kind === "one_to_one") reasons.push(`금액 정확히 일치 (${formatMoney(deposit.amount)})`);
  else reasons.push(`${requests.length}건 합계가 입금액과 일치 (${formatMoney(deposit.amount)})`);
