...
//
// Direction decides everything. A 입금요청 is money WooofPay owes a brand, so
// for 선매입 it leaves the account as 출금; only 위탁 brands send money in.
// Matching across directions attaches a payout to an unrelated customer
// payment that happens to share the amount.
//
...
export function reconcile({ requests = [], transactions = [], options = {} }) {
  ...
  const pending = requests.map((request) => ({
    raw: request,
    id: request.id,
    // 위탁만 브랜드가 우프에 보내는 입금이고, 나머지는 우프가 지급하는 출금이다.
